...
export const buildWindowRange = (activeTimeSeconds, durationSeconds, mode) => {
...
const timeProximityScore = (segment, activeTimeSeconds) => {
    if (activeTimeSeconds === null || activeTimeSeconds === undefined)
        return 0;
    if (activeTimeSeconds >= segment.startTimeSeconds && activeTimeSeconds <= segment.endTimeSeconds) {
        return 1;
    }
    const distance = Math.min(Math.abs(activeTimeSeconds - segment.startTimeSeconds), Math.abs(activeTimeSeconds - segment.endTimeSeconds));
    return 1 / (1 + distance * 2);
};
export const rankSegmentsForQuestion = ({ segments, questionEmbedding, activeTimeSeconds, }) => {
    const activeSegment = activeTimeSeconds === null || activeTimeSeconds === undefined
        ? null
        : segments.find((segment) => activeTimeSeconds >= segment.startTimeSeconds && activeTimeSeconds <= segment.endTimeSeconds) || null;
    const activeIndex = activeSegment ? segments.findIndex((segment) => segment.id === activeSegment.id) : -1;
    const adjacentSegments = activeIndex >= 0
...
export const pickKeyMomentsForQuestion = ({ keyMoments, activeTimeSeconds, }) => {
    ...
    return [...keyMoments]
        .sort((left, right) => Math.abs(left.timestampSeconds - activeTimeSeconds) - Math.abs(right.timestampSeconds - activeTimeSeconds))
        .slice(0, 3);
};
export const buildRetrievalTrace = ({ processingStatus, mode, activeTimeSeconds, durationSeconds, activeSegment, adjacentSegments, semanticSegments, keyMoments, hasWindowFrames, hasCurrentFrame, hasChatHistory, hasBiomechanicsRules, hasPoseSnapshot, }) => {
    const contextSources = ['global_summary'];
    if (activeSegment)
        contextSources.push('active_segment');
    if (adjacentSegments.length > 0)
        contextSources.push('adjacent_segments');
    if (semanticSegments.length > 0)
        contextSources.push('semantic_segments');
    if (keyMoments.length > 0)
        contextSources.push('key_moments');
    if (hasCurrentFrame)
        contextSources.push('current_frame');
    if (hasWindowFrames && mode !== 'summary')
        contextSources.push('window_frames');
    if (hasChatHistory)
        contextSources.push('chat_history');
    if (hasBiomechanicsRules)
        contextSources.push('biomechanics_rules');
    if (hasPoseSnapshot)
        contextSources.push('pose_snapshot');
    ...
    return {
        ...
        windowRangeSeconds,
        windowFrameTimestamps: windowRangeSeconds && mode !== 'summary'
...
export const summarizeContextSources = (sources) => {
    const labels = [];
    if (sources.includes('global_summary'))
        labels.push('resumen global');
    if (sources.includes('active_segment'))
        labels.push('segmento activo');
    if (sources.includes('adjacent_segments'))
        labels.push('tramo anterior/posterior');
    if (sources.includes('semantic_segments'))
        labels.push('segmentos semánticos');
    if (sources.includes('current_frame'))
        labels.push('frame actual');
    if (sources.includes('window_frames'))
        labels.push('ventana temporal');
    if (sources.includes('key_moments'))
        labels.push('momentos clave');
    if (sources.includes('pose_snapshot'))
        labels.push('pose snapshot');
    return labels.join(' + ');
};
const sectionTitles = {
    es: {
...
export const formatStructuredAnswer = (structured, language) => {
...
const normalizeCompactAnswerText = (value) => value
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
    ...
    .replace(/\s+/g, ' ')
    .trim();
const areCompactIdeasSimilar = (left, right) => {
...
const dedupeCompactIdeas = (items, limit, blocked = []) => {
    const selected = [];
    items
        .map((item) => item.trim())
        .filter(Boolean)
        ...
        if (selected.length >= limit)
            return;
        if (blocked.some((candidate) => areCompactIdeasSimilar(candidate, item)))
            return;
        if (selected.some((candidate) => areCompactIdeasSimilar(candidate, item)))
            return;
        selected.push(item);
    });
    return selected;
};
const joinCompactIdeas = (items) => items.join('; ');
const isCompactMeaningfulLimitation = (item) => {
    const normalized = normalizeCompactAnswerText(item);
    if (!normalized)
        return false;
    return !(normalized.includes('sin limitaciones') ||
        normalized.includes('no relevant limitations') ||
        normalized.includes('no hay limitaciones'));
};
export const formatCompactStructuredAnswer = (structured, language) => {
    const observations = dedupeCompactIdeas(structured.visibleObservations || [], 2);
    const technicalPoints = dedupeCompactIdeas([...(structured.technicalEvaluation || []), ...(structured.probableErrorsOrRisks || [])], 2, observations);
    ...
    const limitation = dedupeCompactIdeas((structured.visualLimitations || []).filter(isCompactMeaningfulLimitation), 1)[0];
    if (language === 'ing') {
        const lines = [`${structured.momentOfGesture} (${structured.phase}).`];
        if (observations.length > 0)
            lines.push(`Visible: ${joinCompactIdeas(observations)}.`);
        if (technicalPoints.length > 0)
            lines.push(`Key point: ${joinCompactIdeas(technicalPoints)}.`);
        if (recommendations.length > 0)
            lines.push(`Main adjustment: ${joinCompactIdeas(recommendations)}.`);
    ...
    if (language === 'eus') {
        const lines = [`${structured.momentOfGesture} (${structured.phase}).`];
        if (observations.length > 0)
            lines.push(`Ikusten dena: ${joinCompactIdeas(observations)}.`);
        if (technicalPoints.length > 0)
            lines.push(`Gako teknikoa: ${joinCompactIdeas(technicalPoints)}.`);
        if (recommendations.length > 0)
            lines.push(`Doikuntza nagusia: ${joinCompactIdeas(recommendations)}.`);
        if (inferences.length > 0 && structured.confidence !== 'high') {
    ...
    const lines = [`${structured.momentOfGesture} (${structured.phase}).`];
    if (observations.length > 0)
        lines.push(`Se ve: ${joinCompactIdeas(observations)}.`);
    if (technicalPoints.length > 0)
        lines.push(`Clave técnica: ${joinCompactIdeas(technicalPoints)}.`);
    if (recommendations.length > 0)
        lines.push(`Ajuste principal: ${joinCompactIdeas(recommendations)}.`);
    if (inferences.length > 0 && structured.confidence !== 'high') {
