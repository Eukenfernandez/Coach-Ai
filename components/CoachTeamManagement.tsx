...
export const CoachTeamManagement: React.FC<CoachTeamManagementProps> = ({ currentUser, onSelectAthlete, activeAthleteId, language }) => {
   ...
   const loadAthletes = async () => {
      // Fetch fresh data from Firestore to get the latest managedAthletes list
      // (in case athlete just accepted a request after the coach logged in)
      try {
         ...
         console.warn("Could not fetch fresh coach data, using local state", e);
      }

      // Fallback to local state if Firestore fetch fails
      if (currentUser.profile?.managedAthletes) {
         const list = await StorageService.getManagedAthletes(currentUser.profile.managedAthletes);
         setAthletes(list);
      } else {
         setAthletes([]);
      }
