...
import { ThrowRecord, UserProfile, Language } from '../types';
...
const TEXTS = {
  es: {
    title: 'Competiciones',
    best: 'Mejor',
    progression: 'Progresión de Temporada',
    register: 'Registrar Competición',
    location: 'Sede / Campeonato',
    date: 'Fecha',
    save: 'Guardar Resultado',
    official: 'Resultados Oficiales',
    empty: 'No hay competiciones registradas.',
    graphEmpty: 'Añade al menos 2 competiciones para ver tu progresión.',
    time: 'Tiempo',
    mark: 'Marca'
  },
  ing: {
    title: 'Competitions',
    best: 'Best',
    progression: 'Season Progression',
    register: 'Register Competition',
    location: 'Venue / Championship',
    date: 'Date',
    save: 'Save Result',
    official: 'Official Results',
    empty: 'No competitions registered.',
    graphEmpty: 'Add at least 2 competitions to see your progression.',
    time: 'Time',
    mark: 'Mark'
  },
  eus: {
    title: 'Txapelketak',
    best: 'Onena',
    progression: 'Denboraldi Progresioa',
    register: 'Erregistratu Txapelketa',
    location: 'Egoitza / Txapelketa',
    date: 'Data',
    save: 'Gorde Emaitza',
    official: 'Emaitza Ofizialak',
    empty: 'Ez dago txapelketarik erregistratuta.',
    graphEmpty: 'Gehitu gutxienez 2 txapelketa zure progresioa ikusteko.',
    time: 'Denbora',
    mark: 'Marka'
  }
};

export const JavelinTracker: React.FC<CompetitionTrackerProps> = ({ profile, records, onAddRecord, onDeleteRecord, language }) => {
  ...
  const label = isTimeBased ? t.time : t.mark;
  ...
  const renderGraph = () => {
    ...
    // Use viewBox scaling instead of fixed pixel width to ensure fit on mobile
    const height = 300;
    const width = 800;
    const padding = 40;
             ...
             {sortedRecords.map((rec, i) => {
               ...
               return (
                 <g key={rec.id} className="group">
                    <circle cx={x} cy={y} r="5" className="fill-white dark:fill-neutral-900 stroke-orange-500 stroke-2 md:group-hover:r-7 transition-all cursor-pointer" />
                    {/* Tooltip only on larger screens/hover capable */}
    ...
    <div className="h-full bg-gray-50 dark:bg-neutral-950 p-4 md:p-10 overflow-y-auto transition-colors duration-300">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-6">
          ...
          <div className="bg-white dark:bg-neutral-900 px-4 py-3 md:px-6 md:py-3 rounded-xl border border-orange-500/20 shadow-sm dark:shadow-none w-full md:w-auto transition-colors">
            <span className="block text-[10px] md:text-xs text-neutral-500 dark:text-neutral-400 uppercase tracking-wider mb-1">{t.best} {label} (PB)</span>
            <div className="flex items-baseline gap-1">
               <span className="text-2xl md:text-3xl font-bold text-orange-600 dark:text-orange-500">{bestRecord}</span>
               <span className="text-lg md:text-xl text-orange-600/80 dark:text-orange-500/80">{unit}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="bg-white dark:bg-neutral-900 p-4 md:p-6 rounded-2xl border border-neutral-200 dark:border-neutral-800 h-fit shadow-sm dark:shadow-none transition-colors">
            <h3 className="text-neutral-900 dark:text-white font-semibold mb-4 text-sm md:text-base">{t.register}</h3>
          ...
          <div className="lg:col-span-2 space-y-3">
             <h3 className="text-neutral-900 dark:text-white font-semibold mb-3 md:mb-4 text-sm md:text-base">{t.official}</h3>
             {records.length === 0 && <p className="text-neutral-500 text-sm">{t.empty}</p>}
             {[...records].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(record => (
               <div key={record.id} className="flex items-center justify-between bg-white dark:bg-neutral-900 p-3 md:p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 hover:border-orange-500/30 transition-colors shadow-sm dark:shadow-none">
                  <div className="flex items-center gap-3 md:gap-4">
                     <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-orange-100 dark:bg-orange-500/10 flex items-center justify-center text-orange-600 dark:text-orange-500 font-bold shrink-0">
                        {isTimeBased ? <Timer size={16} className="md:w-[18px]" /> : <Trophy size={16} className="md:w-[18px]" />}
                     </div>
                     <div>
                        <div className="flex items-baseline gap-1">
