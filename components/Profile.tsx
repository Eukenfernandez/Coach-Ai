...
import { User as UserIcon, Save, Trash2, Activity, AlertTriangle, ChevronRight, Check } from 'lucide-react';
...
export const Profile: React.FC<ProfileProps> = ({ currentUser, onUpdateUser, onLogout, language, onRefreshData }) => {
  ...
  const [step, setStep] = useState<'idle' | 'confirm_change' | 'confirm_data'>('idle');
  ...
  const finalizeSportChange = async (keepData: boolean) => {
     ...
     try {
        if (keepData) {
           ...
           const newProfile = { ...formData, sport: pendingSport, discipline: '' } as UserProfile;
